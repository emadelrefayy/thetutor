...
const StudentDashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadTerms = async () => {
      ...
      try {
        const data =
          await apiClient.getTerms(selectedGrade);
  ...
  useEffect(() => {
    ...
    const loadSubjects = async () => {
      ...
      try {
        const data =
          await apiClient.getSubjects(selectedTerm);
    ...
    >
      ...
      <section className="space-y-2">
        ...
        {loadingGrades ? (
          ...
          <div
            id="grade-selector"
            className="flex items-center gap-2 overflow-x-auto scrollbar-none py-2"
          >
            {grades.map((grade) => (
              <button
                key={grade.id}
              ...
              (subject, index) => (
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <div
                      className="w-10 h-10 shrink-0 rounded-xl bg-slate-800 border border-slate-700 text-amber-400 flex items-center justify-center font-bold text-xs group-hover:bg-amber-500 group-hover:text-slate-950 transition-all"
                      aria-hidden="true"
                    >
                      {subject.icon_name ??
                        index + 1}
                    ...
                    <div className="min-w-0">
                      <h3 className="text-sm font-black text-slate-100 group-hover:text-amber-400 transition-all truncate">
                        {subject.title}
                      </h3>
                  ...
                  <span
                    className="text-xs text-amber-400 shrink-0 mr-2"
                    aria-hidden="true"
                  >
                    ◀
                  </span>
