...
<Link
to={"${subjectsPath}?tenantId=${encodeURIComponent( state.context.tenantId, )}&studentProfileId=${encodeURIComponent( state.context.studentProfileId, )}"}
state={{
tenantId:
state.context.tenantId,
studentProfileId:
state.context.studentProfileId,
gradeId:
parsedGradeId,
termId:
parsedTermId,
...
* At this point TypeScript knows that state is the
* ready variant, so units, unitGames, context and
* subjectTitle are all guaranteed to exist.
  */
  const {
  units,
  unitGames,
